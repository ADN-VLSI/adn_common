module adn_common_pipeline_tb #(
  ...
  initial begin
    ...
    data_in_valid_i <= 1'b0;
    @(posedge clk_i);
    assert (data_out_o == 32'hDEADBEEF) else $error("Test 1 failed: data mismatch");
    assert (data_out_valid_o) else $error("Test 1 failed: valid not asserted");
    ...
    for (int i = 0; i < 5; i++) begin
      ...
      assert (data_out_o == i[DATA_WIDTH-1:0]) else $error("Test 2 failed at iteration %0d", i);
    end
    ...
    @(posedge clk_i);
    while (!data_out_valid_o) @(posedge clk_i);
    assert (data_out_o == 32'hCAFEBABE) else $error("Test 3 failed: data mismatch");
