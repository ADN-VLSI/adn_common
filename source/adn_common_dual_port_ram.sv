...
module adn_common_dual_port_ram #(
    ...
    parameter bit OUT_REG    = 1'b0     // 0 = Unregistered (1 cycle latency), 1 = Registered (2 cycle latency)
) (
    // PORTS
  ...
  // Read pipeline registers: Intermediate storage for read data and optional output register
  logic [DATA_WIDTH-1:0] ram_data_out; // Raw output from memory array
  logic [DATA_WIDTH-1:0] ram_data_reg; // Registered output for timing improvement
  ...
  // Synchronous Read Channel (rd_clk_i Domain): Fetches data from memory array based on read address
  ...
  // Output Pipeline Register Stage (OUT_REG): Optional stage to improve read access timing
  always_ff @(posedge rd_clk_i) begin
    if (!rd_rst_n_i) begin
      ram_data_reg <= '0;
    end else begin
  ...
  // Continuous assignment output multiplexer: Selects between raw and registered output
