...
`ifdef SIMULATION
  logic [DATA_WIDTH-1:0] mem_core[longint];
`else
...
`ifdef SIMULATION
  always @(posedge clk_i iff wr_en_i) mem_core[wr_addr_i] = #1step wr_data_i;
`else
...
`ifdef SIMULATION
  always @(clk_i or rd_addr_i) rd_data_o <= mem_core[rd_addr_i];
`else
