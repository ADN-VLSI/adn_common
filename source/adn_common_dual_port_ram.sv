...
### Purpose
This module implements a synchronous dual-port RAM with independent write and read clock domains. It supports configurable data and address widths, and provides an optional output register stage to improve timing performance at the cost of an additional clock cycle of latency.

### Usage
To instantiate the module, define the `DATA_WIDTH` and `ADDR_WIDTH` parameters to match your memory requirements. Set `OUT_REG` to `1` if you need an additional pipeline stage for higher frequency operation (at the cost of 2-cycle read latency).
...
module adn_common_dual_port_ram #(
  ...
  // Synchronous Read Channel (rd_clk_i Domain): Handles data retrieval from the memory array
  ...
  // Output Pipeline Register Stage (OUT_REG): Optional stage to improve timing slack
  always_ff @(posedge rd_clk_i) begin
    if (!rd_rst_n_i) begin
      ram_data_reg <= '0;
    end else begin
  ...
  // Continuous assignment output multiplexer: Selects between raw or registered output
